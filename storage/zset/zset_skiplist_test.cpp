...
TEST_F(ZSetSkiplistTest, Rank) {
  auto r1 = zset_skiplist->Rank("key1");
  ASSERT_EQ(r1.value_or(-1), 3);

  auto r2 = zset_skiplist->Rank("key2");
  ASSERT_EQ(r2.value_or(-1), 2);

  auto r3 = zset_skiplist->Rank("key3");
  ASSERT_EQ(r3.value_or(-1), 0);

  auto r4 = zset_skiplist->Rank("key4");
  ASSERT_EQ(r4.value_or(-1), 1);

  auto r5 = zset_skiplist->Rank("key_not_exist");
...
TEST_F(ZSetSkiplistTest, Update) {
  ...
  ASSERT_EQ(zset_skiplist->Size(), 4);
  auto r0 = zset_skiplist->Rank("key1");
  ...
  r1 = zset_skiplist->Rank("key1");
  ASSERT_EQ(r1, 3);

  r4 = zset_skiplist->Rank("key4");
  ASSERT_EQ(r4, 0);
}
...
TEST_F(ZSetSkiplistTest, RangeByRank) {
  ...
  const auto p0 = ToKeyScorePairs(zset_skiplist->RangeByRank(&spec0));
  ...
  const RangeByRankSpec spec1(1, 3, true, false,
                              std::make_unique<LimitSpec>(0, -1), false);
  const auto p1 = ToKeyScorePairs(zset_skiplist->RangeByRank(&spec1));
  ...
  const RangeByRankSpec spec2(1, 3, false, true,
                              std::make_unique<LimitSpec>(0, -1), false);
  const auto k2 = zset_skiplist->RangeByRank(&spec2);
  ...
  const RangeByRankSpec spec3(1, 3, true, true,
                              std::make_unique<LimitSpec>(0, -1), false);
  const auto p3 = ToKeyScorePairs(zset_skiplist->RangeByRank(&spec3));
  ...
  const auto p4 = ToKeyScorePairs(zset_skiplist->RangeByRank(&spec4));
  ...
  const auto p5 = ToKeyScorePairs(zset_skiplist->RangeByRank(&spec5));
  ...
  const auto p6 = ToKeyScorePairs(zset_skiplist->RangeByRank(&spec6));
  ...
  const auto p7 = ToKeyScorePairs(zset_skiplist->RangeByRank(&spec7));
  ASSERT_EQ(p7.size(), 0);

  // Invalid spec non-exclusive, min > max.
  const RangeByRankSpec spec8(2, 1, false, false,
                              std::make_unique<LimitSpec>(0, -1), false);
  const auto k8 = zset_skiplist->RangeByRank(&spec8);
  ...
  const RangeByRankSpec spec9(1, 1, false, true,
                              std::make_unique<LimitSpec>(0, -1), false);
  const auto p9 = ToKeyScorePairs(zset_skiplist->RangeByRank(&spec9));
  ASSERT_EQ(p9.size(), 0);

  // Min out of range
  const RangeByRankSpec spec10(10, 12, false, false,
                               std::make_unique<LimitSpec>(0, -1), false);
  const auto p10 = ToKeyScorePairs(zset_skiplist->RangeByRank(&spec10));
  ASSERT_EQ(p10.size(), 0);

  // Offset out of range.
  const RangeByRankSpec spec11(0, 3, false, false,
                               std::make_unique<LimitSpec>(10, -1), false);
  const auto p11 = ToKeyScorePairs(zset_skiplist->RangeByRank(&spec11));
  ASSERT_EQ(p11.size(), 0);

  // Negative index
  const RangeByRankSpec spec12(-3, -1, false, false,
                               std::make_unique<LimitSpec>(0, -1), false);
  const auto p12 = ToKeyScorePairs(zset_skiplist->RangeByRank(&spec12));
  const auto e12 =
  ...
  const RangeByRankSpec spec13(-3, -1, true, false,
                               std::make_unique<LimitSpec>(0, -1), false);
  const auto p13 = ToKeyScorePairs(zset_skiplist->RangeByRank(&spec13));
  ...
  const RangeByRankSpec spec14(-3, -1, false, true,
                               std::make_unique<LimitSpec>(0, -1), false);
  const auto p14 = ToKeyScorePairs(zset_skiplist->RangeByRank(&spec14));
  ...
  const RangeByRankSpec spec15(-3, -1, false, false,
                               std::make_unique<LimitSpec>(0, -1), true);
  const auto p15 = ToKeyScorePairs(zset_skiplist->RangeByRank(&spec15));
  const auto e15 =
...
TEST_F(ZSetSkiplistTest, RangeByScore) {
  ...
  const RangeByScoreSpec spec0(1.0, 5.0, false, false,
                               std::make_unique<LimitSpec>(0, -1), false);
  const auto p0 = ToKeyScorePairs(zset_skiplist->RangeByScore(&spec0));
  const auto e0 = std::vector<KeyScorePair>{
      {"key4", 1.0}, {"key2", 2.0}, {"key3", 4.0}, {"key1", 5.0}};
  ASSERT_EQ(p0, e0);
  ...
  const RangeByScoreSpec spec1(-std::numeric_limits<double>::infinity(),
                               ...
                               false);
  const auto p1 = ToKeyScorePairs(zset_skiplist->RangeByScore(&spec1));
  ...
  const RangeByScoreSpec spec2(2.0, 5.0, true, false,
                               std::make_unique<LimitSpec>(0, -1), false);
  const auto p2 = ToKeyScorePairs(zset_skiplist->RangeByScore(&spec2));
  ...
  const RangeByScoreSpec spec3(2.0, 5.0, false, true,
                               std::make_unique<LimitSpec>(0, -1), false);
  const auto p3 = ToKeyScorePairs(zset_skiplist->RangeByScore(&spec3));
  ...
  const RangeByScoreSpec spec4(2.0, 5.0, true, true,
                               std::make_unique<LimitSpec>(0, -1), false);
  const auto p4 = ToKeyScorePairs(zset_skiplist->RangeByScore(&spec4));
  ...
  const auto p5 = ToKeyScorePairs(zset_skiplist->RangeByScore(&spec5));
  ...
  const RangeByScoreSpec spec6(2.0, 6.0, false, false,
                               std::make_unique<LimitSpec>(0, -1), true);
  const auto p6 = ToKeyScorePairs(zset_skiplist->RangeByScore(&spec6));
  const auto e6 =
  ...
  const RangeByScoreSpec spec7(1.0, 6.0, false, false,
                               std::make_unique<LimitSpec>(1, 2), true);
  const auto p7 = ToKeyScorePairs(zset_skiplist->RangeByScore(&spec7));
...
TEST_F(ZSetSkiplistTest, Delete) {
  ...
  auto rank = zset_skiplist->Rank("key1");
